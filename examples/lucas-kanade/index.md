...
Note that in SSDA if we adopt as our threshold the minimum error we have found among the h examined so far, we obtain an algorithm similar to alpha-beta pruning in minmax game trees [@Nilsson1971]. Here we take advantage of the fact that in evaluating $min_h \sum_x d(x, h)$, where $d(x, h)$ is the contribution of pixel $x$ at disparity $h$ to the total error, the $\sum_x$ can only increase as we look at more $x$’s (more pixels).
...
::: figure {#fig2}
```js
html`<img src="assets/fig2.svg" style="width:50%;">`;
```
...
~~~ math
\Delta c \approx \left[\sum_x \left(\frac{\partial q}{\partial c} \frac{\partial F}{\partial q} \right)^T [G-F] \right] \left[ \left(\frac{\partial q}{\partial c} \frac{\partial F}{\partial q} \right)^T \left(\frac{\partial q}{\partial c} \frac{\partial F}{\partial q} \right) \right]^{-1}
~~~
